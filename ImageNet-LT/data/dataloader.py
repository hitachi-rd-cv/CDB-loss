...
class LT_Dataset(Dataset):
    ...
    def __init__(self, root, txt, transform=None):
        ...
        with open(txt) as f:
            for line in f:
                image_path = line.split()[0].replace('train', 'ILSVRC2012_img_train')
                if 'val' in image_path:
                       image_name = image_path.split('/')[-1]
                       image_path = os.path.join('ILSVRC2012_img_val', image_name)
                self.img_path.append(os.path.join(root, image_path))
...
# Load datasets
def load_data(data_root, dataset, phase, batch_size, sampler_dic=None, cls_diff=None, num_workers=4, test_open=False, shuffle=True):
