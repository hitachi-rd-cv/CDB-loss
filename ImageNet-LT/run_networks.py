...
Portions of the source code are from the classifier-balancing project which
...
class model ():
    ...
    def __init__(self, config, data, test=False):
        ...
        self.class_difficulty = np.ones(self.training_opt['num_classes'])
        self.is_cdb = False
        # Setup logger
    ...
    def init_criterions(self):
        ...
        for key, val in criterion_defs.items():
            ...
            if 'CDBSoftmaxLoss' in def_file:
                self.is_cdb = True
                self.criterions[key] = source_import(def_file).create_loss(self.class_difficulty, loss_args).to(self.device)
            else:
    ...
    def batch_loss(self, labels):
        ...
        if 'PerformanceLoss' in self.criterions.keys():
            self.loss_perf = torch.mean(self.criterions['PerformanceLoss'](self.logits, labels))
        ...
        if 'FeatureLoss' in self.criterions.keys():
            self.loss_feat = torch.mean(self.criterions['FeatureLoss'](self.features, labels))
        ...
        for epoch in range(1, end_epoch + 1):
            ...
            for step, (inputs, labels, indexes) in enumerate(self.data['train']):
                ...
                if hasattr(self.data['train'].sampler, 'update_weights'):
                    ...
                    if self.training_opt['sampler']['type'] in ['ClassPrioritySampler']:
    ...
    def eval(self, phase='val', openset=False, save_feat=False):
        ...
        if phase == 'val':
            print_write(print_str, self.log_file)
            if self.is_cdb:
                self.class_difficulty = 1 - np.array(self.cls_accs) ## update class-wise difficulty
                self.init_criterions()
        else:
